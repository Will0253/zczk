        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            <nav className="flex items-center gap-2 text-white/60 text-sm mb-8">
              ...
              <Link to="/solutions" className="hover:text-[#fdbd00] transition-colors">解决方案</Link>
              <ChevronRight className="w-3 h-3" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {hardwareList.map((item, i) => (
              ...
              >
                ...
                <div className="mb-8 aspect-video rounded-2xl overflow-hidden bg-[#11345b]/20 border border-white/5 group-hover:border-[#fdbd00]/30 transition-colors">
                  <ImageWithFallback 
                    src={item.image} 
                    alt={item.name}
      ...
      <section className="py-24 bg-slate-50/50 overflow-hidden border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            ...
            {/* Middle Row: Flow */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12 items-stretch relative">
              {/* 1. Identify Problem */}
              <motion.div
                initial={{ opacity: 0, x: -20 }}
                whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true }}
                className="bg-white rounded-[40px] p-8 md:p-10 border border-gray-100 shadow-xl shadow-gray-200/50 flex flex-col group"
              >
                <div className="flex items-center gap-4 mb-10">
                  ...
                  <div>
                    <h4 className="text-xl font-black text-gray-900">识别问题</h4>
                    <p className="text-xs text-gray-400 uppercase font-bold tracking-widest">Identify Level</p>
                  </div>
                </div>

                <div className="space-y-4 mb-10 flex-1">
                  {[
                    { icon: Video, label: '监控摄像头', color: 'text-blue-500', bg: 'bg-blue-50' },
                    { icon: Radio, label: '雷达系统', color: 'text-cyan-500', bg: 'bg-cyan-50' },
                    { icon: CloudRain, label: '风雨传感器', color: 'text-blue-400', bg: 'bg-blue-50' }
                  ].map((dev, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-gray-50/50 hover:bg-white hover:shadow-md border border-transparent hover:border-gray-100 rounded-2xl transition-all">
                      <div className="flex items-center gap-4">
                        <div className={`w-8 h-8 ${dev.bg} rounded-lg flex items-center justify-center`}>
                ...
                <div className="pt-8 border-t border-gray-50">
                  <div className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-4">外部触发源</div>
                  <div className="flex gap-2">
                    <span className="px-4 py-1.5 bg-blue-50 text-blue-600 text-[10px] font-black rounded-full border border-blue-100/50">事件检测</span>
                    <span className="px-4 py-1.5 bg-red-50 text-red-600 text-[10px] font-black rounded-full border border-red-100/50">危险车辆</span>
                  </div>
                </div>
              </motion.div>

              {/* 2. Warning Platform (Center) */}
              <motion.div
                initial={{ opacity: 0, scale: 0.95 }}
                whileInView={{ opacity: 1, scale: 1 }}
                viewport={{ once: true }}
              ...
              >
                {/* Horizontal Connection Particles */}
                <div className="hidden lg:block absolute right-full top-1/2 -translate-y-1/2 w-12 h-px bg-blue-100">
                  <motion.div 
                ...
                <div className="hidden lg:block absolute left-full top-1/2 -translate-y-1/2 w-12 h-px bg-blue-100">
                  <motion.div 
                    animate={{ left: ['0%', '100%'], opacity: [0, 1, 0] }}
                ...
                <div className="flex-1 bg-gradient-to-br from-[#11345b] to-[#1a4a7a] rounded-[48px] p-10 md:p-12 text-center relative overflow-hidden group shadow-2xl shadow-blue-900/20">
                  {/* Decorative Elements */}
                  <div className="absolute top-0 right-0 w-32 h-32 bg-blue-400/10 rounded-full blur-3xl" />
                  <div className="absolute bottom-0 left-0 w-32 h-32 bg-[#fdbd00]/5 rounded-full blur-3xl" />

                  <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-md border border-white/10 text-[#fdbd00] rounded-full mb-12">
                    <Radio className="w-3.5 h-3.5" />
                    <span className="text-xs font-black uppercase tracking-widest">New Gen 4G Link</span>
                  </div>

                  <div className="relative mb-10 group-hover:scale-105 transition-transform duration-500">
                    <div className="w-32 h-32 mx-auto bg-white/5 rounded-full flex items-center justify-center backdrop-blur-sm border border-white/10">
                      ...
                      <div className="absolute -top-1 -right-1 w-10 h-10 bg-[#fdbd00] rounded-full flex items-center justify-center shadow-lg shadow-[#fdbd00]/20">
                        <RefreshCw className="w-5 h-5 text-[#11345b] animate-spin-slow" />
                      </div>
                    </div>
                  </div>

                  <h4 className="text-3xl font-black text-white mb-6">预警平台</h4>
                  <p className="text-sm text-white/60 leading-relaxed mb-12 px-4">
                    行业领先的实时数据处理中心，实现亚秒级分析与指令分发。
                  </p>

                  <div className="grid grid-cols-2 gap-4 text-[10px] font-black text-white/40 uppercase tracking-widest bg-black/20 p-4 rounded-2xl border border-white/5">
                    ...
                    <div className="flex flex-col items-center gap-2">
                      <div className="w-2 h-2 bg-[#fdbd00] rounded-full mb-1" />
                      信号分发
                    </div>
                  </div>
                </div>
              ...
              >
                <div className="flex items-center justify-between mb-10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-xl font-black text-gray-900">执行预警</h4>
                      <p className="text-xs text-gray-400 uppercase font-bold tracking-widest">Execute Level</p>
                    </div>
                  </div>
                  <div className="px-4 py-1.5 bg-[#11345b] text-[#fdbd00] rounded-lg text-[10px] font-black shadow-lg shadow-blue-900/10">
                    126 dB MAX
                  </div>
                </div>

                <div className="relative space-y-10 pl-8 flex-1">
                  {/* Vertical dotted line */}
                  <div className="absolute left-[47px] top-6 bottom-6 w-px border-l-2 border-dashed border-gray-200" />
                  
                  {[
                    { id: 'A', status: 'Online', desc: 'Master Unit' },
                    { id: 'B', status: 'Online', desc: '100m Interval' },
                    { id: 'C', status: 'Online', desc: '100m Interval' }
                  ].map((unit, i) => (
                    <div key={i} className="relative z-10 flex items-center gap-6 group/unit">
                      <div className="w-10 h-10 bg-white border-2 border-blue-50 rounded-full flex items-center justify-center shadow-sm group-hover/unit:border-blue-500 group-hover/unit:scale-110 transition-all duration-300">
                        <Volume2 className="w-5 h-5 text-blue-600" />
                      </div>
                      <div className="flex-1 bg-gray-50/50 p-4 rounded-2xl border border-transparent hover:border-gray-100 transition-colors">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-black text-gray-900 tracking-tight">扬声器单元 {unit.id}</span>
                          <span className="text-[10px] text-green-500 font-black uppercase">{unit.status}</span>
                        </div>
                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{unit.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <div className="relative">
                  <div className="rounded-3xl overflow-hidden shadow-2xl border-4 border-gray-50">
                    <ImageWithFallback src={slowLaneImg} className="w-full h-auto" />
                  </div>
            ...
            <div className="bg-white rounded-[60px] p-8 md:p-12 shadow-xl border border-gray-100 overflow-hidden">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div className="order-2 lg:order-1 relative">
                  <div className="rounded-3xl overflow-hidden shadow-2xl border-4 border-gray-50">
                    <ImageWithFallback src={fastLaneImg} className="w-full h-auto" />
                  </div>
          ...
          <div className="bg-[#11345b] rounded-[60px] p-12 md:p-20 text-white relative overflow-hidden">
            ...
            <div className="max-w-3xl relative z-10">
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <button className="px-10 py-5 bg-white/10 text-white font-black rounded-3xl flex items-center justify-center gap-3 border border-white/20 hover:bg-white/20 transition-all">
                  预约专家视频演示
