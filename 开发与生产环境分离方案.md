...
生产环境：
├── 前端：https://domain.com (通过 Nginx)
├── 后端：https://domain.com/api (通过 Nginx)
├── 数据库：PostgreSQL (Docker Volume)
└── SSL：Let's Encrypt 自动续期
```

**域名：**
- 生产域名：根据实际部署配置（默认 example.com）
...
| `ops/.env.prod.example` | 生产环境变量示例 | ✅ 已存在 | ✅ |
| `ops/.env.dev.example` | 开发环境变量示例 | ✅ 已存在 | ✅ |
| `ops/.env.prod` | 生产环境配置 | ⚠️ 需创建 | ❌ |
...
| 目标 | 描述 | 验证标准 |
|------|------|----------|
| **开发环境** | 使用 SQLite，快速启动，支持热重载，便于功能迭代 | 启动时间 < 10s，支持热重载 |
| **生产环境** | 使用 PostgreSQL，容器化部署，高可用，数据持久化 | 99.9% 可用性，数据自动备份 |
| **配置隔离** | 开发和生产环境配置完全独立，互不影响 | 配置文件分离，环境变量独立 |
| **安全合规** | 敏感信息不提交到代码仓库，生产环境使用 HTTPS | 无敏感信息泄露，HTTPS 强制跳转 |
| **自动化** | 部署流程自动化，减少人工操作错误 | 一键部署，自动证书续期 |

### 2.2 原则

1. **配置分离原则**：不同环境使用不同的配置文件，避免配置污染
2. **最小权限原则**：生产环境只开放必要的端口和服务，使用非 root 用户运行容器
3. **数据安全原则**：敏感信息加密存储，定期备份，使用强密码和密钥
4. **可追溯原则**：所有操作记录日志，便于问题排查和审计
5. **自动化优先原则**：尽可能使用脚本和工具自动化操作，减少人为错误
6. **高可用原则**：关键服务配置健康检查和自动重启机制
...
│   ├── .env                    # 开发环境配置（不提交）
│   ├── .env.example            # 开发环境示例
│   ├── Dockerfile              # 后端容器镜像（ops/docker/Dockerfile.backend）
...
│   ├── .env                    # 开发环境配置（不提交）
│   ├── .env.example            # 开发环境示例
│   ├── Dockerfile              # 前端容器镜像（ops/docker/Dockerfile.frontend）
│   ├── .dockerignore           # Docker 忽略文件
...
│   ├── dev.sh                  # 开发环境启动脚本
│   ├── deploy.sh               # 生产环境部署脚本（需创建）
│   ├── generate-secrets.sh     # 生成密钥脚本（需创建）
│   ├── init-cert.sh            # 证书初始化脚本（需创建）
│   ├── docker/
...
| 目录/文件 | 来源 | 用途 | 应排除 |
|-----------|------|------|--------|
| `backend/dist/` | Strapi 构建 | TypeScript 编译产物 | ✅ |
| `backend/build/` | Strapi 构建 | 生产构建产物 | ✅ |
| `frontend/.next/` | Next.js 构建 | 构建缓存和输出 | ✅ |
| `frontend/out/` | Next.js 构建 | 静态导出产物 | ✅ |
| `node_modules/` | 依赖安装 | 依赖包 | ✅ |
...
```dockerfile
# 先复制 package 文件，利用 Docker layer cache
COPY backend/package.json backend/package-lock.json* ./
RUN npm install

# 后复制源代码，源代码变化不会导致重新安装依赖
COPY backend/ ./
RUN npm run build
```
...
| 优化措施 | 效果 |
|---------|------|
| 使用 Alpine 基础镜像 | 减少镜像大小约 50% |
| 多阶段构建 | 最终镜像只包含运行时文件 |
| 清理 npm cache | 减少镜像大小约 20% |
| 非 root 用户 | 提高安全性 |
...
# 2. 配置环境变量
./ops/generate-secrets.sh example.com
...
```bash
# 查看后端构建产物
docker run --rm zczk-backend ls -la /app/backend/dist
docker run --rm zczk-backend ls -la /app/backend/build

# 查看前端构建产物
docker run --rm zczk-frontend ls -la /app/frontend/.next
```

**验证镜像大小：**
```bash
# 查看镜像大小
docker images | grep zczk
```
...
**修改 `ops/docker/certbot-renew.sh`：**
```bash
#!/bin/sh
set -eu
...
**修改 `ops/docker/docker-compose.prod.yml` 的 certbot 服务：**
```yaml
certbot:
  ...
  volumes:
    ...
    - ./init-cert.sh:/usr/local/bin/init-cert.sh:ro
    - ./certbot-renew.sh:/usr/local/bin/certbot-renew.sh:ro
    - ./cron.txt:/etc/crontabs/root:ro
...
docker compose -f ops/docker/docker-compose.prod.yml run --rm \
    -e DOMAINS="your-domain.com,www.your-domain.com" \
    -e LETSENCRYPT_EMAIL="admin@your-domain.com" \
    certbot init-cert.sh

# 4. 启动 nginx 和 certbot
...
# 5. 检查证书是否存在
if ! docker compose -f docker/docker-compose.prod.yml exec -T nginx ls /etc/letsencrypt/live/example.com/fullchain.pem 2>/dev/null; then
...
**告警通知方式：**
- 邮件通知：通过 Let's Encrypt 邮箱
- 日志记录：记录到容器日志
- 监控面板：可选集成 Prometheus + Grafana

**告警处理流程：**
```
告警触发 → 记录日志 → 发送通知 → 自动恢复（如重启容器） → 人工介入（如需要）
```
...
RUN mkdir -p /app/backend/public/uploads && \
    chown -R strapi:nodejs /app/backend
...
curl -f https://your-domain.com/api/news
```
...
export PROD_STRAPI_URL=https://your-domain.com
...
**`backend/scripts/import-dev-seed.mjs` 功能：**

| 功能 | 说明 |
|------|------|
| **Products 导入** | 从开发环境获取所有产品数据，包括图片和图库 |
| **News 导入** | 从开发环境获取所有新闻数据，包括图片和附件 |
| **Media 上传** | 自动下载开发环境的媒体文件并上传到生产环境 |
| **Upsert 逻辑** | 如果 slug 已存在则更新，否则创建新记录 |
| **批量处理** | 支持分页获取大量数据 |
| **错误处理** | 详细的错误日志和失败重试 |
...
┌─────────────────────────────────────────────────────────────┐
│  1. 从开发环境获取数据（分页）                        │
│     ├─ Fetch Products (page 1, 2, 3...)              │
│     └─ Fetch News (page 1, 2, 3...)                 │
└─────────────────────────────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────┐
│  3. 上传媒体文件到生产环境                              │
│     └─ 使用 FormData 上传到 /api/upload               │
└─────────────────────────────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────┐
│  4. 创建/更新内容到生产环境                            │
│     ├─ Upsert Products (检查 slug 是否存在)             │
│     └─ Upsert News (检查 slug 是否存在)                │
└─────────────────────────────────────────────────────────────┘
```
...
# 检查生产环境产品数量
PROD_PRODUCT_COUNT=$(curl -s https://your-domain.com/api/products | jq 'meta.pagination.total')
...
open https://your-domain.com/news

# 检查产品详情页
open https://your-domain.com/products/product-1

# 检查新闻详情页
open https://your-domain.com/news/news-1
```
...
curl https://example.com/api/news
...
### 18.1 `ops/docker/docker-compose.prod.yml`
...
services:
  ...
  backend:
    ...
    volumes:
      - strapi-uploads:/app/backend/public/uploads
    depends_on:
      - postgres
    healthcheck:
      test: ["CMD-SHELL", "wget --quiet --tries=1 --spider http://localhost:1337/api/healthz || exit 1"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 10s
  ...
  frontend:
    ...
    depends_on:
      - backend
    healthcheck:
      test: ["CMD-SHELL", "wget --quiet --tries=1 --spider http://localhost:3000/healthz || exit 1"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 10s
  ...
  nginx:
    ...
    volumes:
      ...
      - ../nginx/sites.conf:/etc/nginx/conf.d/sites.conf:ro
...
### 16.2 `ops/docker/docker-compose.dev.yml`
...
### 16.3 `ops/docker/Dockerfile.backend`
...
RUN mkdir -p /app/backend/public/uploads && chown -R strapi:nodejs /app/backend
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD wget --quiet --tries=1 --spider http://localhost:1337/api/healthz || exit 1
...
### 16.4 `ops/docker/Dockerfile.frontend`
...
location = /api/healthz {
    proxy_pass http://backend_upstream/api/healthz;
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
}
```
