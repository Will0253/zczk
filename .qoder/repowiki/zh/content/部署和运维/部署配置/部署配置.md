...
### 数据库配置与迁移
- 数据库配置：后端支持 MySQL、PostgreSQL、SQLite 三种数据库，通过 DATABASE_CLIENT 环境变量切换。
- 开发环境：默认使用 SQLite，文件位于 backend/.tmp/data.db。
- 生产环境：使用 PostgreSQL，通过 DATABASE_URL 或主机、端口、用户名、密码配置。
- 迁移流程：提供专用脚本进行开发到生产的数据库迁移，支持分批迁移和数据完整性验证。

**章节来源**
- file://backend/config/database.ts#L1-L61
- file://DEPLOYMENT.md#L747-L941
- file://开发与生产环境分离方案.md#L350-L426

### 安全配置
- CORS 配置：Strapi 后端配置跨域访问白名单，支持开发和生产域名。
- 网络访问控制：仅开放 80、443 端口给公网，其他服务端口仅容器内部访问。
- 权限控制：容器以非 root 用户运行，PostgreSQL 使用强密码。
- API 访问控制：通过 API Token 进行访问控制，迁移后及时删除临时 Token。

**章节来源**
- file://backend/config/middlewares.ts#L1-L13
- file://backend/config/server.ts#L1-L8
- file://DEPLOYMENT.md#L1022-L1096
- file://开发与生产环境分离方案.md#L371-L426
